    ...
    public class MovementCharacterController : MonoBehaviour
    {
        ...
        private void CheckGroundStatus()
        {
            ...
            Debug.DrawLine(
                ...
                transform.position + Vector3.down * (_controller.height / 2 + 0.05f),
                Color.red
            );

#endif
            Vector3 origin = transform.position + Vector3.up * 0.1f; // чуть выше ног
            float distance = _controller.height / 2 + 0.05f; // увеличиваем
            bool grounded = Physics.Raycast(origin, Vector3.down, distance);
